...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    const pool = new Pool({ connectionString: databaseUrl, max: 3, idleTimeoutMillis: 10000, connectionTimeoutMillis: 15000, ssl: /@(localhost|127\.0\.0\.1)/.test(databaseUrl) ? undefined : { rejectUnauthorized: false } });
